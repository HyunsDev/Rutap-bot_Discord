...
import asyncio, discord, os, requests, random, datetime, re, json, sys, setting
...
async def on_ready():
    ...
    while a < 1:
        embed=discord.Embed(title="I'm online!", description="%s/%s/%s | %s:%s" % (str(now.year), str(now.month), str(now.day), str(now.hour), str(now.minute)), color=0xb2ebf4)
...
async def on_message(message):
    try:
        try:
            ...
            if os.path.isfile("%s_Banned.rts" % (message.author.id)):
                return None
            ...
            if os.path.isfile("Server_%s/%s_Server_prefix.rts" % (message.server.id, message.server.id)):
                ...
                if message.content.startswith('rutap admin debug'):
                    if message.content == "rutap admin debug help":
                        ...
                        if message.author.id == Setting.owner_id:
                            n = await app.send_message(message.channel, "<@%s>, 파일 정보를 불러오는 중 입니다. 최장 5초가 소요됩니다." % (message.author.id))
                            q = message.content[26:]
                            try:
                                content = open(q, 'r').read()
                                await app.delete_message(n)
                                await app.send_message(message.channel, "<@%s>, `%s` 파일의 내용입니다 : \n```%s```" % (message.author.id, q, content))
                            except Exception as e:
                                await app.delete_message(n)
                                await app.send_message(message.channel, "<@%s>, 해당 파일이 존재하지 않습니다.\n\n```%s```" % (message.author.id, e))
                        else:
                            await app.send_message(message.channel, "<@%s>, 봇 관리자로 등록되어 있지 않습니다. `setting.py` 파일을 확인하여 주세요." % (message.author.id))
                ...
                if message.content.startswith('rutap admin notice -s channel'):
                    if message.author.id == Setting.owner_id:
                        ...
                        try:
                            ...
                            await app.send_message(message.channel, "<@%s>, `%s`에 메시지를 보내지 못하였습니다." % (message.author.id, channel_info, e))
                ...
                if message.content.startswith(prefix + '이미지'):
                    ...
                    google_data = requests.get("https://www.google.co.kr/search?q=" + group + "&source=lnms&tbm=isch&sa=X")
                ...
                if os.path.isfile("Server_%s/%s_Server_first.rtl" % (message.server.id, message.server.id)):
                    if message.content == "루탑봇 활성화":
                        if message.author.server_permissions.administrator or message.author.id == Setting.owner_id:
                            ...
                            open("Server_%s/%s_Server_CC_Limit.rtl" % (message.server.id, message.server.id), 'w').write("0")
